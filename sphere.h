...
private:
    static void get_light_uv(const point3& p, double& u, double& v) {
        //p: a given point on the light of radius one, centered at the origin. 
        //u: returned value [0,1] of angle around the Y axis from X=-1 
        //v: returned value [0,1] of angle from Y = -1 to Y = +1
        // <1 0 0> yields <0.50 0.50>         <-1 0 0> yields <0.00 0.50>
        // <0 1 0> yields <0.50 1.00>         <0 -1 0> yields <0.50 0.00>
...
bool light::hit(const ray& r, double tmin, double tmax, hit_record& rec) const {
    vec3 oc = r.origin() - center;
    double a = r.direction().lenght_squared();
    double half_b = dot(oc, r.direction());
    double c = oc.lenght_squared() - (radius * radius);
    ...
    vec3 outward_normal = (rec.p - center) / radius;
    rec.set_face_normal(r, outward_normal);
    get_light_uv(outward_normal, rec.u, rec.v);
    rec.mat_ptr = mat_ptr;

    return true;
}

bool light::bounding_box(double time0, double time1, aabb& output_box) const {
    output_box = aabb(
        center - vec3(radius, radius, radius),
        center + vec3(radius, radius, radius));
    return true;
